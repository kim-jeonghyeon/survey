...
<body>
	...
	<main>
		...
		<c:forEach items="${survey}" var="item">
			<ul id="survey_bottom">
				<li><a href="/surveyresult?s_idx=${item.s_idx}">
		...
		<div>
			<ul>
				<c:if test = "${pagination.page > 1}">
					<li>
						<a href="/resultlist?page=${pagination.prevPage}&f=${pagination.search.field}&q=${pagination.search.query}">◀</a>
					</li>
				</c:if> 
			<c:forEach var="i" begin="${pagination.startPage}" end="${pagination.endPage}" step="1">
						...
						<c:if test="${ pagination.page ne i }">
							<li>
								<a href="/resultlist?page=${i}&f=${pagination.search.field}&q=${pagination.search.query}">${i}</a>
							</li>
						</c:if>
			</c:forEach>
				<c:if test = "${pagination.page < pagination.lastPage}">
					<li>
						<a href="/resultlist?page=${pagination.lastPage}&f=${pagination.search.field}&q=${pagination.search.query}">▶</a>
					</li>
				</c:if>
		</ul>
		</div>
